...
class DataDiklatKaryawanByTopik extends CI_Controller {
	...
	{
		$data = array();
		$data["headerTitle"] = "Diklat by Topik";
		$data["title"] = "Data Diklat Karyawan by Nama Topik";
		$data["dview"] = "v_contents_dataDiklatKaryawanByTopik_search";
		$data["segment"] = $this->uri->segment(1);
		$navbar = array("Home", "Data Diklat Karyawan", "by Nama Topik");
	...
	public function listData(){
		...
		$data['jumlahData'] = $this->DataDiklatKaryawanByTopikModel->count_all();
		$res = $this->DataDiklatModel->getLastUpdated();
		$result = $res->row();
		$data["lastUpdated"] = date("d/m/Y", strtotime($result->DTM_UPD));
		$this->load->view('v_contents_dataDiklatKaryawanByTopik', $data);
	}
	...
	public function getXlsx(){
		$data = array();
		$dataTbl = $this->DataDiklatKaryawanByTopikModel->selectAll();		
		...
		$excel->setActiveSheetIndex(0)->setCellValue('B1', "TOPIK");
		$excel->setActiveSheetIndex(0)->setCellValue('C1', "SUBYEK");
		$excel->setActiveSheetIndex(0)->setCellValue('D1', "JENIS DIKLAT");
		...
		foreach($dataTbl as $row){
			...
			$excel->setActiveSheetIndex(0)->setCellValue('B'.$i, $row->ALIAS);
			$excel->setActiveSheetIndex(0)->setCellValue('C'.$i, $row->NAMA_SUBYEK);
			$excel->setActiveSheetIndex(0)->setCellValue('D'.$i, $row->JENIS_DIKLAT); 
		...
		header('Content-Disposition: attachment; filename="Data Diklat '.$_GET["valTopik"].' '.date("dmY").'.xlsx"'); 
    ...
    public function ajax_list(){
		$list = $this->DataDiklatKaryawanByTopikModel->get_datatables();
		$data = array();
		$no = $_POST['start'];
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->DataDiklatKaryawanByTopikModel->count_all(),
			"recordsFiltered" => $this->DataDiklatKaryawanByTopikModel->count_filtered(),
			"data" => $data,
		);
	...
	function getPdf(){		
		...
		$this->pdf->selectFont(APPPATH . '/third_party/pdf-php/fonts/Helvetica.afm');
			...
			$this->pdf->ezText("<b>\"".$_GET['valTopik']."\"</b>", 12, array('justification' => "center"));
			...
			$fooo['<b>TOPIK</b>'] = array('justification' => "center", 'width'=>"130");
			...
			$this->pdf->ezStream(array("Content-Disposition"=>"Data Diklat ".$_GET['valTopik']." ".date('dmY').".pdf"));
    }
