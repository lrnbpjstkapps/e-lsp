...
class DataDiklatKaryawanBySubyek extends CI_Controller {
	...
	{
		$data = array();
		$data["headerTitle"] = "Diklat by Subyek";
		$data["title"] = "Data Diklat Karyawan by Nama Subyek";
		$data["dview"] = "v_contents_dataDiklatKaryawanBySubyek_search";
		$data["segment"] = $this->uri->segment(1);
		$navbar = array("Home", "Data Diklat Karyawan", "by Nama Subyek");
		...
		$namaSubyek = $this->DataDiklatKaryawanBySubyekModel->selectDistinctByAColumn("JENIS_SUBYEK", "NAMA_SUBYEK", "ASC");
		$data['namaSubyek'] = $namaSubyek;
		$this->load->view('v_layout', $data);
	}
	
	public function hei(){
		$data = array();
		if($_POST['jenisDiklat']!='-- Select All --'){
			$namaSubyek = $this->DataDiklatKaryawanBySubyekModel->selectDistinctByJenisDiklat($_POST['jenisDiklat']);
		}else{
			$namaSubyek = $this->DataDiklatKaryawanBySubyekModel->selectDistinctByAColumn("JENIS_SUBYEK", "NAMA_SUBYEK", "ASC");
		}
		foreach($namaSubyek->result() as $row){
	...
	public function listData(){
		$data = array();
		$data['namaSubyek'] = $this->input->post("namaSubyek");
		$data['jumlahData'] = $this->DataDiklatKaryawanBySubyekModel->count_all();
		$res = $this->DataDiklatModel->getLastUpdated();
		$result = $res->row();
		$data["lastUpdated"] = date("d/m/Y", strtotime($result->DTM_UPD));
		$this->load->view('v_contents_dataDiklatKaryawanBySubyek', $data);
	}
			...
			foreach ($objWorksheet->getRowIterator() as $row){				
				if($i>0&&""!=($objWorksheet->getCell('B'.($i+1))->getValue())){
					...
					if(!$result->num_rows()>0){
						...
						$this->DataDiklatKaryawanBySubyekModel->insert($dataTbl);
					}else{
						$dataTbl['NAMA_PEGAWAI'] = $objWorksheet->getCell('C'.($i+1))->getValue();
						$dataTbl['UNIT_KERJA'] = $objWorksheet->getCell('L'.($i+1))->getValue();
						$dataTbl['WILAYAH'] = $objWorksheet->getCell('H'.($i+1))->getValue();
						...
						$this->DataDiklatKaryawanBySubyekModel->update($dataTbl['NPP'], $dataTbl);
					}
				}
				$i++;				
			}
			
			$result = $this->DataDiklatKaryawanBySubyekModel->selectAll("DTM_CRT","ASC");
			foreach($result->result() as $row){
				if(!in_array($row->NPP, $listNPP)){
					$this->DataDiklatKaryawanBySubyekModel->delete($row->NPP, "ADMIN");
				}
			}
	...
	public function getXlsx(){
		$data = array();
		$dataTbl = $this->DataDiklatKaryawanBySubyekModel->selectAll();		
		...
		$excel->setActiveSheetIndex(0)->setCellValue('B1', "SUBYEK");
		$excel->setActiveSheetIndex(0)->setCellValue('C1', "TOPIK");
		...
		foreach($dataTbl as $row){
			...
			$excel->setActiveSheetIndex(0)->setCellValue('C'.$i, $row->ALIAS);
		...
		header('Content-Disposition: attachment; filename="Data Diklat '.$_GET["namaSubyek"].' '.date("dmY").'.xlsx"'); 
    ...
    public function ajax_list(){
		$list = $this->DataDiklatKaryawanBySubyekModel->get_datatables();
		$data = array();
		$no = $_POST['start'];
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->DataDiklatKaryawanBySubyekModel->count_all(),
			"recordsFiltered" => $this->DataDiklatKaryawanBySubyekModel->count_filtered(),
			"data" => $data,
		);
	...
	function getPdf(){		
		...
		$this->pdf->selectFont(APPPATH . '/third_party/pdf-php/fonts/Helvetica.afm');
			...
			$this->pdf->ezText("<b>\"".$_GET['namaSubyek']."\"</b>", 12, array('justification' => "center"));
			...
			$_POST['namaSubyek'] = $_GET['namaSubyek'];
			$list = $this->DataDiklatKaryawanBySubyekModel->get_datatables();
			$dataTbl = array();
			$i=0;
			foreach ($list as $row) {				
			...
			$this->pdf->ezStream(array("Content-Disposition"=>"Data Diklat ".$_GET['namaSubyek']." ".date('dmY').".pdf"));
    }
