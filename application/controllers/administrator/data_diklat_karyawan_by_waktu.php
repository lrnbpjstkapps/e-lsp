...
class DataDiklatKaryawanByWaktu extends CI_Controller {

	public function __construct(){
		parent::__construct();
		$this->load->model("DataDiklatModel");
		$this->load->model("DataDiklatKaryawanBySubyekModel");
		$this->load->model("DataDiklatKaryawanByTopikModel");
		$this->load->model("DataDiklatKaryawanByWaktuModel");
	...
	{
		$data = array();
		$data["headerTitle"] = "Diklat by Waktu";
		$data["title"] = "Data Diklat Karyawan by Waktu";
		$data["dview"] = "v_contents_dataDiklatKaryawanByWaktu_search";
		$data["segment"] = $this->uri->segment(1);
		
		$navbar = array("Home", "Data Diklat Karyawan", "by Waktu");
		...
		$listSubyek = $this->DataDiklatKaryawanBySubyekModel->selectDistinctByAColumn("JENIS_SUBYEK", "NAMA_SUBYEK", "ASC");
		$data['listSubyek'] = $listSubyek;
		$listTopik = $this->DataDiklatKaryawanByTopikModel->selectDistinctByAColumn("JENIS_TOPIK", "NAMA_TOPIK", "ASC");
		$data['listTopik'] = $listTopik;
		$this->load->view('v_layout', $data);
	}
	
	public function getListSubyek(){
		$data = array();
		if($_POST['valJenisDiklat']!='-- Select All --'){
			$listSubyek = $this->DataDiklatKaryawanBySubyekModel->selectDistinctByJenisDiklat($_POST['valJenisDiklat']);
		}else{
			$listSubyek = $this->DataDiklatKaryawanBySubyekModel->selectDistinctByAColumn("JENIS_SUBYEK", "NAMA_SUBYEK", "ASC");
		}
		if($listSubyek->num_rows()>0){
			echo "<option> -- Select All -- </option>";
			foreach($listSubyek->result() as $row){
				echo "<option>  ".$row->NAMA_SUBYEK." </option>";
			}
		}
		exit;	
	}	
	...
	public function getListTopik(){
		$data = array();
		if($_POST['valSubyek']!='-- Select All --'){
			$listTopik = $this->DataDiklatKaryawanByTopikModel->selectDistinctByJenisSubyek($_POST['valSubyek']);
		}else{
			$listTopik = $this->DataDiklatKaryawanByTopikModel->selectDistinctByAColumn("JENIS_TOPIK", "NAMA_TOPIK", "ASC");
		}
		
		if($listTopik->num_rows()>0){
			echo "<option> -- Select All -- </option>";
			foreach($listTopik->result() as $row){
				echo "<option>  ".$row->NAMA_TOPIK." </option>";
			}
		}
		exit;	
	}	
	...
	public function listData(){
		...
		//parameters to filter records
		$data['valTopik'] 		= $this -> input -> post("valTopik");
		$data['valSubyek'] 		= $this -> input -> post("valSubyek");
		$data['valJenisDiklat'] = $this -> input -> post("valJenisDiklat");
		$data['valTglMulai'] 	= $this -> input -> post("valTglMulai");
		$data['valTglSelesai'] 	= $this -> input -> post("valTglSelesai");		
		...
		//whether download file or not
		$data['jumlahData'] 	= $this -> DataDiklatKaryawanByWaktuModel -> count_all();
		
		//show page fragment
		$this->load->view('v_contents_dataDiklatKaryawanByWaktu', $data);
	}
			...
			foreach ($objWorksheet->getRowIterator() as $row){				
				if($i>0&&""!=($objWorksheet->getCell('B'.($i+1))->getValue())){
					...
					if(!$result->num_rows()>0){
						...
						$dataTbl['USR_CRT'] = "ADMIN";
						$this->DataDiklatKaryawanByTopikModel->insert($dataTbl);
					}else{
						$dataTbl['NAMA_PEGAWAI'] = $objWorksheet->getCell('C'.($i+1))->getValue();
						$dataTbl['UNIT_KERJA'] = $objWorksheet->getCell('L'.($i+1))->getValue();
						$dataTbl['WILAYAH'] = $objWorksheet->getCell('H'.($i+1))->getValue();
						...
						$this->DataDiklatKaryawanByTopikModel->update($dataTbl['NPP'], $dataTbl);
					}
				}
				$i++;				
			}
			
			$result = $this->DataDiklatKaryawanByTopikModel->selectAll("DTM_CRT","ASC");
			foreach($result->result() as $row){
				if(!in_array($row->NPP, $listNPP)){
					$this->DataDiklatKaryawanByTopikModel->delete($row->NPP, "ADMIN");
				}
			}
	...
	public function getXlsx(){
		$data = array();
		$dataTbl = $this->DataDiklatKaryawanByWaktuModel->selectAll();		
		...
		$excel->setActiveSheetIndex(0)->setCellValue('B1', "JENIS DIKLAT");
		$excel->setActiveSheetIndex(0)->setCellValue('C1', "SUBYEK");
		$excel->setActiveSheetIndex(0)->setCellValue('D1', "TOPIK");
		$excel->setActiveSheetIndex(0)->setCellValue('E1', "NPP"); 
		$excel->setActiveSheetIndex(0)->setCellValue('F1', "NAMA PEGAWAI");
		$excel->setActiveSheetIndex(0)->setCellValue('G1', "KANTOR WILAYAH");
		$excel->setActiveSheetIndex(0)->setCellValue('H1', "UNIT KERJA");
		...
		foreach($dataTbl as $row){
			...
			$excel->setActiveSheetIndex(0)->setCellValue('B'.$i, $row->JENIS_DIKLAT);
			$excel->setActiveSheetIndex(0)->setCellValue('C'.$i, $row->NAMA_SUBYEK);
			$excel->setActiveSheetIndex(0)->setCellValue('D'.$i, $row->ALIAS); 
			$excel->setActiveSheetIndex(0)->setCellValue('E'.$i, $row->NPP); 
			$excel->setActiveSheetIndex(0)->setCellValue('F'.$i, $row->NAMA_PEGAWAI); 
			$excel->setActiveSheetIndex(0)->setCellValue('G'.$i, $row->WILAYAH); 
			$excel->setActiveSheetIndex(0)->setCellValue('H'.$i, $row->UNIT_KERJA);
		...
		header('Content-Disposition: attachment; filename="Data Diklat '.date("dmY").'.xlsx"'); 
    ...
    public function ajax_list(){
		...
		$list = $this -> DataDiklatKaryawanByWaktuModel -> get_datatables();		
		$no = $_POST['start'];
		foreach ($list as $obj) {
			$row = array();
			$row[] = ++$no;			
			$row[] = $obj -> JENIS_DIKLAT;			
			$row[] = $obj -> NAMA_SUBYEK;
			$row[] = $obj -> ALIAS;
			$row[] = $obj -> NPP;
			$row[] = $obj -> NAMA_PEGAWAI;
			$row[] = $obj -> WILAYAH;
			$row[] = $obj -> UNIT_KERJA;	
			$row[] = $obj -> TANGGAL_MULAI;	
			$row[] = $obj -> TANGGAL_SELESAI;				
			$row[] = $obj -> GOLONGAN;
			$row[] = $obj -> NILAI;
			$data[] = $row;
		}
		...
		$output = array(
			"draw" 				=> $_POST['draw'],
			"recordsTotal" 		=> $this -> DataDiklatKaryawanByWaktuModel -> count_all(),
			"recordsFiltered" 	=> $this -> DataDiklatKaryawanByWaktuModel -> count_filtered(),
			"data" 				=> $data,
		);
	...
	function getPdf(){		
		...
		$this->pdf->selectFont(APPPATH . '/third_party/pdf-php/fonts/Helvetica.afm');
            ...
            $this->pdf->ezText("<b>DATA DIKLAT KARYAWAN BPJS KETENAGAKERJAAN</b>", 12, array('justification' => "center"));
			...
			$fooo['<b>JENIS DIKLAT</b>'] = array('justification' => "center", 'width'=>"130");
			$fooo['<b>SUBYEK</b>'] = array('justification' => "center", 'width'=>"100");
			$fooo['<b>TOPIK</b>'] = array('justification' => "center", 'width'=>"55");
			...
			$_POST['valSubyek'] = $_GET['valSubyek'];
			$_POST['valTopik'] = $_GET['valTopik'];
			$_POST['valJenisDiklat'] = $_GET['valJenisDiklat'];
			$_POST['valTglMulai'] = $_GET['valTglMulai'];
			$_POST['valTglSelesai'] = $_GET['valTglSelesai'];
			$list = $this->DataDiklatKaryawanByWaktuModel->get_datatables();
			$dataTbl = array();
			$i=0;
			foreach ($list as $row) {				
			...
			$this->pdf->ezStream(array("Content-Disposition"=>"Data Diklat ".date('dmY').".pdf"));
    }
