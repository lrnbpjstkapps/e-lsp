...
class DataDiklatKaryawanByUnitKerja extends CI_Controller {
	...
	{
		$data = array();
		$data["headerTitle"] = "Diklat by Unit Kerja";
		$data["title"] = "Data Diklat Karyawan by Unit Kerja";
		$data["dview"] = "v_contents_dataDiklatKaryawanByUnitKerja_search";
		$data["segment"] = $this->uri->segment(1);
		$navbar = array("Home", "Data Diklat Karyawan", "by Unit Kerja");
	...
	public function listData(){
		...
		$data['jumlahData'] = $this->DataDiklatKaryawanByUnitKerjaModel->count_all();
		$res = $this->DataDiklatModel->getLastUpdated();
		$result = $res->row();
		$data["lastUpdated"] = date("d/m/Y", strtotime($result->DTM_UPD));
		$this->load->view('v_contents_dataDiklatKaryawanByUnitKerja', $data);
	}
	
	public function getXlsx(){
		$data = array();
		$dataTbl = $this->DataDiklatKaryawanByUnitKerjaModel->selectAll();		
		...
		$excel->setActiveSheetIndex(0)->setCellValue('B1', "UNIT KERJA");
		...
		foreach($dataTbl as $row){
			...
			$excel->setActiveSheetIndex(0)->setCellValue('B'.$i, $row->UNIT_KERJA); 
			$excel->setActiveSheetIndex(0)->setCellValue('C'.$i, $row->WILAYAH); 
    ...
    public function ajax_list(){
		$list = $this->DataDiklatKaryawanByUnitKerjaModel->get_datatables();
		$data = array();
		$no = $_POST['start'];
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->DataDiklatKaryawanByUnitKerjaModel->count_all(),
			"recordsFiltered" => $this->DataDiklatKaryawanByUnitKerjaModel->count_filtered(),
			"data" => $data,
		);
	...
	function getPdf(){		
		...
		$_POST['unitKerja'] = $_GET['unitKerja'];
		$list = $this->DataDiklatKaryawanByUnitKerjaModel->get_datatables();
		$dataTbl = array();
		$i=0;
		foreach ($list as $row) {				
