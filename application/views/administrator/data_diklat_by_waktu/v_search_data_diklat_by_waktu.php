			...
			<div class="box-body pad table-responsive">
				...
				<div class="col-md-3">
					<div class="form-group">
						...
						<input type="text" class="form-control" name="valTglMulai" id="idTglMulai" placeholder="yyyy-mm-dd" data-date-format='yyyy-mm-dd'>
					</div>
				</div>
				<div class="col-md-3">
					<div class="form-group">
						<label for="alias">Tanggal Selesai</label>
						<input type="text" class="form-control" name="valTglSelesai" id="idTglSelesai" placeholder="yyyy-mm-dd" data-date-format='yyyy-mm-dd'>
					</div>
				</div>
			</div>
...
function leaveChange1(control){
	...
	$.ajax({
		type: "POST",
		dataType: "html",
		url: "DataDiklatKaryawanByWaktu/getListSubyek",
		beforeSend: function (){
...
function leaveChange2(valJenisDiklat){
	...
	$.ajax({
		type: "POST",
		dataType: "html",
		url: "DataDiklatKaryawanByWaktu/getListTopik",
		beforeSend: function (){
			$("div#divLoading").addClass('show');      
			$('a').on('click.myDisable', function(e) { e.preventDefault(); });
			$(':button').prop('disabled', true);
...
$(document).ready(function(){
	$('#idTglMulai').datepicker({
		autoclose: true,
		altField:"#idTglMulai",
		onSelect: function () {
			document.getElementById('#idTglMulai').value(this.value);
		},
		endDate: new Date()
    }).on('changeDate', function(selected) {
		var startDate = new Date(selected.date.valueOf());
		$('#idTglSelesai').datepicker({
			autoclose: true,
			altField:"#idTglSelesai",
			onSelect: function () {
				document.getElementById('#idTglSelesai').value(this.value);
			},
			startDate: startDate,
			endDate: new Date()
		});		
        if($("#idTglMulai").val() > $("#idTglSelesai").val()){
          $("#idTglSelesai").val($("#idTglMulai").val());
        }
    });
	
	$("#formCariDataDiklatKaryawan").validate({
		rules: {
			valTglMulai: {
				required: true,
				date: true
		...
		submitHandler: function(form){	
			...
			$.ajax({
				url: 'DataDiklatKaryawanByWaktu/listData',
				type: 'POST',
				data: new FormData($("#formCariDataDiklatKaryawan")[0]),
				beforeSend: function (){
					$("div#divLoading").addClass('show');      
					$('a').on('click.myDisable', function(e) { e.preventDefault(); });
					$(':button').prop('disabled', true);
